...
public class Favourite_Movies_Adapter extends RecyclerView.Adapter<Favourite_Movies_Adapter.MyViewHolder> {

    private Context context;
    private List<User> userList;
    public Favourite_Movies_Adapter(Context context) {
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
        holder.tvMovName.setText(this.userList.get(position).movieName);
        holder.tvReleaseYr.setText(this.userList.get(position).releaseYear);
        holder.movID = this.userList.get(position).movieID;
        String movieIDsend = this.userList.get(position).movieID;
        String movieNameFirst = this.userList.get(position).movieName;
...
//        holder.imgLink = this.userList.get(position).imageLink;
        Picasso.get().load("https://image.tmdb.org/t/p/w500"+this.userList.get(position).imageLinkMovieSmall).into(holder.smallimg);
        holder.itemView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
//                Toast.makeText(view.getContext(), movieIDsend, Toast.LENGTH_SHORT).show();
                Intent intent = new Intent(view.getContext(), Movies_Description_Activity.class);
    ...
    public class MyViewHolder extends RecyclerView.ViewHolder{
        ...
        public MyViewHolder(View view) {
            super(view);
            tvMovName = view.findViewById(R.id.item_title);
            tvReleaseYr = view.findViewById(R.id.item_subText);
